...
def main(args):
    ...
    data_scaler = Scaler(axis=(0, 1, 2)) #初始化归一化模块
    ...
    # --------------------- Trainer setting --------------------
    net_config = model_config["DCP_STGNet"]
    net_config.update(data_config)


    Model = DCP_STGNet.STGNet
    net_pred = Model(**net_config).to(device)
    net_graph = DCP_STGNet.GraphLearn(net_config['num_nodes'], net_config['init_feature_num']).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_config_path', type=str, default='./config/train_PeMS_Orange_County.yaml',
    ...
    parser.add_argument('--model_save_path', type=str, default='./model_states/DCP-STGNet_PeMS_Orange_County.pkl',
