...
from .ASTGCN import TemporalAttention,SpatialAttention
...
class TEST1(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.nodes = nn.Sequential(
            ...
            nn.Conv2d(64, 64, kernel_size=(1, 1)),
            nn.ReLU(True),
            nn.Conv2d(64, f_in, kernel_size=(1, 1))#输出维度可以进行调整
        )
        self.adj_conv = torch.nn.Conv2d(3, 6, (1, c_in))#设定输出维度
    def forward(self, x, adj_mx):

        nodes = self.nodes(x.transpose(1, 3))
        graph_learn = self.adj_conv(F.relu(nodes)).squeeze(3).transpose(1, 2)
        self.dropout(graph_learn)
        m = graph_learn
        # nodes = self.nodes(x.transpose(1, 3)).squeeze(3).transpose(1, 2)
        # self.dropout(nodes)
        # m = nodes
        A_mi = torch.einsum('bud,bvd->buv', [m, m]).mean(axis = 0)
        A_mi = F.relu(F.normalize(A_mi)-0.05)
        A_adj = F.normalize(F.relu(A_mi + adj_mx ), p=1, dim=-1)


        # h = x
        h = nodes.transpose(1, 3)
        for net_block in self.block_list:
            h = net_block(h, A_adj)
        h = self.final_conv(h).squeeze(dim=-1)
        return h
